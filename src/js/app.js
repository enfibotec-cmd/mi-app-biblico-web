...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function debounce(fn, delay = 350) {
    ...
    return (...a) => { 
      ...
      t = setTimeout(() => { t = null; fn(...a); }, delay); 
    };
  }
  ...
  function buildNoteKey(type, bid, cid, extra = '') {
    return `${type}_${String(bid)}_${String(cid)}${extra ? '_' + String(extra) : ''}`;
  }
  ...
  function loadNotesFromStorage() {
    try {
      ...
      setTimeout(() => {
        if (backup && confirm('Tus notas presentan errores. ¿Descargar respaldo antes de reiniciar?')) {
          try {
        ...
        showWarning('Notas reiniciadas de forma segura.');
      }, 500);
      return {};
    }
  }
  ...
  function safeSaveNotes(n) {
    try {
      ...
      if (getByteSize(s) > MAX_STORAGE_SIZE) { 
      ...
      console.error('[Biblia App] Error guardando:', e); 
  ...
  if (!storageListenerAdded) {
    window.addEventListener('storage', (ev) => {
      ...
      try {
        ...
        if (JSON.stringify(merged) !== JSON.stringify(userNotes)) { 
          userNotes = merged; 
          renderPassage(); 
        }
  ...
  async function loadManifest() {
    ...
    try {
      ...
      if (Array.isArray(data.books)) raw = data.books;
        ...
        raw = Object.entries(data.books).map(([k, v]) => 
          isPlainObject(v) ? { ...v, code: v.code || k, id: v.id || k } : null
        ).filter(Boolean);
      }
  ...
  async function fetchBookDetailData(bookId) {
    ...
    try {
      ...
      chapterDataCache.set(k, d); 
      return d;
    } catch (e) { 
      console.warn('[Biblia App] No se pudo cargar ' + bookId, e); 
      return null; 
    }
  }

  function updateBadgeCounts() {
    const ot = booksList.filter(b => b.testament === 'OT').length;
    const nt = booksList.filter(b => b.testament === 'NT').length;
  ...
  function initScopeFilter() {
    ...
    elements.scopeFilter.addEventListener('click', ev => {
      ...
      elements.scopeFilter.querySelectorAll('.scope-btn').forEach(b => {
        const a = b.dataset.scope === currentScope; 
  ...
  function populateBooks() {
    ...
    if (currentScope === 'ALL') {
      ...
      gOT.label = '— Antiguo Testamento —'; 
      gNT.label = '— Nuevo Testamento —';
      fb.forEach(b => {
        const o = new Option(b.name, String(b.code || b.id));
        if (b.testament === 'OT') gOT.appendChild(o); 
        else if (b.testament === 'NT') gNT.appendChild(o); 
        else elements.bookSelect.appendChild(o);
      });
      if (gOT.children.length) elements.bookSelect.appendChild(gOT);
      if (gNT.children.length) elements.bookSelect.appendChild(gNT);
    } else { 
      fb.forEach(b => elements.bookSelect.appendChild(new Option(b.name, String(b.code || b.id)))); 
    }
  ...
  async function updateChapters() {
    ...
    let tc = Number(book?.chapters || book?.chapterCount || 0);
    
    if (tc <= 0) { 
      const cl = await fetchBookDetailData(bid); 
      if (seq !== chapterSequence) return; 
      if (Array.isArray(cl)) tc = cl.length; 
    }
    ...
    const d = new Option('-- Capítulo --', '');
    const opts = Array.from({ length: tc }, (_, i) => new Option('Capítulo ' + (i + 1), String(i + 1)));
    elements.chapterSelect.replaceChildren(d, ...opts); 
  ...
  async function updateVerses() {
    ...
    if (Array.isArray(book.verseCounts) && Number.isInteger(Number(book.verseCounts[chapterIndex]))) {
      ...
      if (Array.isArray(detailData)) {
        ...
        if (chapterObj) {
          if (Array.isArray(chapterObj.verses)) totalVerses = chapterObj.verses.length;
          else if (Array.isArray(chapterObj.verseList)) totalVerses = chapterObj.verseList.length;
          else if (typeof chapterObj.versesCount === 'number') totalVerses = chapterObj.versesCount;
          else if (typeof chapterObj.verseCount === 'number') totalVerses = chapterObj.verseCount;
          else if (typeof chapterObj.numVerses === 'number') totalVerses = chapterObj.numVerses;
          else {
            const verseKeys = Object.keys(chapterObj).filter(k => /^\d+$/.test(k));
            if (verseKeys.length > 0) totalVerses = Math.max(...verseKeys.map(Number));
          }
        }
      }
    }
    ...
    // Eliminada la llamada redundante a renderPassage() aquí, 
    // el event listener de 'change' en verseSelect se encarga.
  }
  ...
  async function renderPassage() {
    if (!elements.passageDisplay) return;
    const cr = ++renderSequence;
    const bid = elements.bookSelect.value;
    const cv = elements.chapterSelect.value;
    const vv = elements.verseSelect.value;
    
    if (!bid) { clearPassageDisplay(); return; }
    const book = booksMap.get(bid); 
    if (!book) { clearPassageDisplay(); return; }
    
    clearAutoSaveTimers();
    const frag = document.createDocumentFragment();
    
    // 1. Encabezado del Pasaje
    const bc = document.createElement('div'); 
    bc.className = 'passage-title-group'; 
    ...
    if (cv) { 
      ...
      if (Array.isArray(cl)) cd = cl.find(c => Number(c.chapter) === Number(cv)) || null; 
    }

    const th = document.createElement('div'); 
    th.className = 'passage-header';
    const tg = document.createElement('div'); 
    tg.className = 'passage-title-group';
    
    const ki = cd?.conceptualSummary?.keyIdea;
    if (typeof ki === 'string' && ki.trim()) { 
      const kb = document.createElement('span'); 
      kb.className = 'badge-accent'; 
      kb.textContent = ki; 
      tg.appendChild(kb); 
    }
    
    let tt = String(book.name); 
    if (cv) tt += ' ' + cv; 
    if (vv) tt += ':' + vv;
    ...
    if (Number.isFinite(rt) && rt > 0) { 
      ...
      m.className = 'meta-info'; 
      m.textContent = '⏱️ ' + rt + ' min de lectura'; 
      th.appendChild(m); 
    }
    frag.appendChild(th);

    // 2. TEXTO DEL PASAJE (NUEVO: Soluciona que no aparecieran los versículos)
    if (cd) {
      const versesContainer = document.createElement('div');
      versesContainer.className = 'verses-text';
      
      let verses = cd.verses || cd.verseList || cd.text || [];
      
      if (Array.isArray(verses)) {
        if (verses.length > 0 && typeof verses[0] === 'object') {
          if (vv) {
            const verseNum = Number(vv);
            const verse = verses.find(v => Number(v.verse || v.number || v.id) === verseNum);
            if (verse) {
              const p = document.createElement('p');
              const sup = document.createElement('sup');
              sup.textContent = verseNum + ' ';
              p.append(sup, document.createTextNode(verse.text || verse.content || ''));
              versesContainer.appendChild(p);
            } else {
              versesContainer.textContent = 'Versículo no encontrado en los datos.';
            }
          } else {
            verses.forEach(v => {
              const p = document.createElement('p');
              const verseNum = v.verse || v.number || v.id;
              const sup = document.createElement('sup');
              sup.textContent = verseNum + ' ';
              p.append(sup, document.createTextNode(v.text || v.content || ''));
              p.style.marginBottom = '0.5rem';
              versesContainer.appendChild(p);
            });
          }
        } else if (verses.length > 0 && typeof verses[0] === 'string') {
          if (vv) {
            const verseNum = Number(vv);
            ...
            if (verseText) {
              ...
              const sup = document.createElement('sup');
              sup.textContent = verseNum + ' ';
              p.append(sup, document.createTextNode(verseText));
              versesContainer.appendChild(p);
            }
          } else {
            verses.forEach((text, i) => {
              ...
              const sup = document.createElement('sup');
              sup.textContent = (i + 1) + ' ';
              p.append(sup, document.createTextNode(text));
              p.style.marginBottom = '0.5rem';
              versesContainer.appendChild(p);
            });
          }
        ...
        versesContainer.textContent = 'Texto del pasaje no disponible en este formato.';
      }
      frag.appendChild(versesContainer);
    }

    // 3. Módulo de Estudio Interactivo
    if (cd) {
      renderInteractiveStudyModule(frag, cd, bid);
    } else { 
      const p = document.createElement('p'); 
      p.style.color = 'var(--text-muted)'; 
      p.style.marginTop = '0.75rem'; 
      p.textContent = 'Selecciona un capítulo para ver el material de estudio.'; 
      frag.appendChild(p); 
    }
    
    if (cr !== renderSequence) return;
    elements.passageDisplay.replaceChildren(frag); 
  ...
  function renderInteractiveStudyModule(parent, cd, bid) {
    ...
    const pSid = buildNoteKey('panel', bid, cid, 'summary');
    const pQid = buildNoteKey('panel', bid, cid, 'questions');
    const pEid = buildNoteKey('panel', bid, cid, 'exercise');

    const nav = document.createElement('nav'); 
    nav.className = 'passage-tabs'; 
    nav.setAttribute('role', 'tablist'); 
    nav.setAttribute('aria-label', 'Secciones del pasaje');
    ...
    const tS = mkTab(pSid, '📝 Resumen', true);
    const qc = Array.isArray(cd.criticalAnalysisQuestions) ? cd.criticalAnalysisQuestions.length : 0;
    const tQ = mkTab(pQid, '🤔 Análisis (' + qc + ')');
    const tE = mkTab(pEid, '🎯 Práctica');
    nav.append(tS, tQ, tE); 
    parent.appendChild(nav);

    // RESUMEN
    const pS = document.createElement('article'); 
    pS.id = pSid; 
    pS.className = 'tab-content'; 
    pS.setAttribute('role', 'tabpanel'); 
    pS.setAttribute('aria-labelledby', tS.id);
    const pTxt = document.createElement('p'); 
    pTxt.className = 'summary-text'; 
    pTxt.textContent = typeof cd?.conceptualSummary?.text === 'string' ? cd.conceptualSummary.text : 'Sin resumen disponible.';
    const ft = document.createElement('footer'); 
    ft.className = 'summary-footer';
    const wc = Number(cd?.conceptualSummary?.wordCount);
    const sm = document.createElement('small'); 
    sm.className = 'text-muted'; 
    sm.textContent = 'Palabras: ' + (Number.isFinite(wc) ? wc : 0);
    ft.appendChild(sm); 
    pS.append(pTxt, ft); 
    parent.appendChild(pS);

    // ANÁLISIS (PREGUNTAS)
    const pQ = document.createElement('article'); 
    pQ.id = pQid; 
    pQ.className = 'tab-content hidden'; 
    pQ.setAttribute('role', 'tabpanel'); 
    pQ.setAttribute('aria-labelledby', tQ.id);
    ...
    qs.forEach(q => {
      ...
      const nk = buildNoteKey('q', bid, cid, qid);
      const sv = getNoteValue(nk);
      const st = typeof sv === 'string' ? sv : '';
      ...
      bf.textContent = 'Enfoque: ' + (typeof q.focus === 'string' ? q.focus : 'General'); 
      qh.appendChild(bf);
      
      const qb = document.createElement('p'); 
      qb.className = 'question-body';
      const sn = document.createElement('strong'); 
      sn.textContent = qid + '. '; 
      qb.append(sn, document.createTextNode(typeof q.question === 'string' ? q.question : ''));
      
      const tid = 'ta_' + nk;
      const lb = document.createElement('label'); 
      lb.className = 'visually-hidden'; 
      lb.setAttribute('for', tid); 
      lb.textContent = 'Reflexión pregunta ' + qid;
      
      const ta = document.createElement('textarea'); 
      ta.className = 'form-control question-input'; 
      ta.id = tid; 
      ta.dataset.key = nk;
      ta.maxLength = MAX_NOTE_LENGTH; 
      ta.placeholder = 'Escribe tu reflexión sobre este punto...'; 
      ta.value = st;
      ...
      ss.dataset.statusKey = nk; 
      ss.textContent = st ? '✓ Guardado' : 'Listo para guardar';
      
      card.append(qh, qb, lb, ta, ss); 
      qList.appendChild(card);
    });
    pQ.appendChild(qList); 
    parent.appendChild(pQ);

    // PRÁCTICA
    const pE = document.createElement('article'); 
    pE.id = pEid; 
    pE.className = 'tab-content hidden'; 
    pE.setAttribute('role', 'tabpanel'); 
    pE.setAttribute('aria-labelledby', tE.id);
    const ec = document.createElement('div'); 
    ec.className = 'exercise-card';
    const eh = document.createElement('div'); 
    eh.className = 'exercise-header';
    const h3 = document.createElement('h3'); 
    h3.textContent = 'Objetivo del Ejercicio';
    const sd = document.createElement('span'); 
    sd.className = 'badge-time';
    const dur = Number(cd?.practicalExercise?.suggestedDurationMinutes); 
    sd.textContent = '⌛ ' + (Number.isFinite(dur) && dur > 0 ? dur : 15) + ' min';
    eh.append(h3, sd);
    
    const po = document.createElement('p'); 
    po.className = 'exercise-objective'; 
    po.textContent = cd?.practicalExercise?.objective || 'Sin objetivo definido.';
    
    const h4 = document.createElement('h4'); 
    h4.className = 'section-subtitle'; 
    h4.textContent = 'Pasos de Implementación';
    ...
    ins.forEach((st, i) => {
      const sk = buildNoteKey('step', bid, cid, i);
      const sv = getNoteValue(sk);
      const ic = sv === true;
      
      const li = document.createElement('li'); 
      li.className = 'checklist-item';
      const la = document.createElement('label'); 
      la.className = 'checkbox-label';
      const cb = document.createElement('input'); 
      cb.type = 'checkbox'; 
      cb.dataset.key = sk; 
      cb.checked = ic;
      const sp = document.createElement('span'); 
      sp.textContent = typeof st === 'string' ? st : '';
      la.append(cb, sp); 
      li.appendChild(la); 
      ul.appendChild(li);
    });
    ...
    db.className = 'deliverable-box';
    const dl = document.createElement('label'); 
    dl.className = 'deliverable-label';
    const di = document.createElement('span'); 
    di.textContent = '📋 '; 
    const ds = document.createElement('strong'); 
    ds.textContent = 'Entregable Generado'; 
    dl.append(di, ds);
    
    const dd = document.createElement('p'); 
    dd.className = 'deliverable-desc'; 
    dd.textContent = cd?.practicalExercise?.deliverable || '';
    
    const dk = buildNoteKey('deliv', bid, cid);
    const dv = getNoteValue(dk);
    const dt = typeof dv === 'string' ? dv : '';
    const daid = 'ta_' + dk;
    
    const dlb = document.createElement('label'); 
    dlb.className = 'visually-hidden'; 
    dlb.setAttribute('for', daid); 
    dlb.textContent = 'Entregable final';
    
    const da = document.createElement('textarea'); 
    da.className = 'form-control'; 
    da.id = daid; 
    da.dataset.key = dk;
    da.maxLength = MAX_NOTE_LENGTH; 
    da.placeholder = 'Escribe aquí tu plan de acción o entregable final...'; 
    da.value = dt;
    da.setAttribute('autocomplete', 'off'); 
    da.setAttribute('spellcheck', 'true');
    
    const dss = document.createElement('span'); 
    dss.className = 'save-status'; 
    dss.dataset.statusKey = dk; 
    dss.textContent = dt ? '✓ Guardado' : 'Listo para guardar';
    
    db.append(dl, dd, dlb, da, dss);
    ec.append(eh, po, h4, ul, db); 
    pE.appendChild(ec); 
  ...
  function initPassageDisplayDelegation() {
    ...
    elements.passageDisplay.addEventListener('click', ev => {
      ...
      const pid = tb.getAttribute('aria-controls'); 
      if (!pid) return;
      const tp = elements.passageDisplay.querySelector('#' + CSS.escape(pid)); 
      if (tp) tp.classList.remove('hidden');
    });

    elements.passageDisplay.addEventListener('change', ev => {
      const t = ev.target; 
      if (!(t instanceof HTMLInputElement) || t.type !== 'checkbox' || !t.dataset.key) return;
      userNotes[t.dataset.key] = { value: t.checked, _ts: now() }; 
      saveNotesToStorage();
    });

    elements.passageDisplay.addEventListener('input', ev => {
      const t = ev.target; 
      if (!(t instanceof HTMLTextAreaElement) || !t.dataset.key) return;
      const k = t.dataset.key; 
      userNotes[k] = { value: t.value, _ts: now() };
      
      const se = elements.passageDisplay.querySelector('[data-status-key="' + CSS.escape(k) + '"]');
      if (se) { 
        se.textContent = '⏳ Guardando...'; 
      ...
      const tid = setTimeout(() => { 
        if (se) { 
          se.textContent = '✓ Guardado'; 
          setTimeout(() => se.classList.remove('visible'), 1500); 
        } 
    ...
    elements.passageDisplay.addEventListener('keydown', ev => {
      const t = ev.target; 
      if (!(t instanceof HTMLButtonElement) || !t.matches('.tab-btn')) return;
      const tabs = Array.from(elements.passageDisplay.querySelectorAll('.tab-btn'));
      const ci = tabs.indexOf(t); 
      if (ci === -1) return;
      
      let ni = ci;
      if (ev.key === 'ArrowRight') ni = (ci + 1) % tabs.length; 
      else if (ev.key === 'ArrowLeft') ni = (ci - 1 + tabs.length) % tabs.length;
  ...
  function resetSelect(sel, ph) { 
    if (!sel) return; 
    sel.replaceChildren(new Option(ph, '')); 
    sel.disabled = true; 
  }
  ...
  function showError(msg, retry) {
    ...
    s.textContent = '⚠️ ' + msg; 
    elements.statusBanner.appendChild(s);
    if (typeof retry === 'function') { 
      ...
      elements.statusBanner.appendChild(b); 
    }
  ...
  function exportNotes() {
    try {
      const b = new Blob([JSON.stringify(userNotes, null, 2)], { type: 'application/json' });
      ...
      console.error(e); 
      showError('No se pudieron exportar.', null); 
    }
  }
  ...
  function importNotesFromFile(file) {
    ...
    if (file.size > MAX_STORAGE_SIZE) {
      showError('El archivo es demasiado grande.', null);
      return;
    }
    ...
    r.onload = (e) => {
      try {
        ...
        if (!isPlainObject(imp)) throw new Error('Formato inválido');
        
        // Validación básica contra prototype pollution
        if (imp.__proto__ || imp.constructor || imp.prototype) {
          throw new Error('El archivo contiene claves prohibidas por seguridad.');
        }
        ...
        if (confirm('¿Reemplazar notas actuales?\nAceptar = Reemplazar\nCancelar = Fusionar')) {
          ...
          for (const [k, v] of Object.entries(ni)) { 
            const ex = m[k]; 
            if (!ex) m[k] = v; 
            else if (isPlainObject(ex) && isPlainObject(v) && typeof ex._ts === 'number' && typeof v._ts === 'number') {
              m[k] = v._ts >= ex._ts ? v : ex; 
            } else {
              m[k] = v; 
            }
          } 
        ...
        safeSaveNotes(userNotes); 
        renderPassage(); 
        showWarning('✅ Notas importadas.');
      } catch (err) { 
        console.error(err); 
        showError('Archivo inválido: ' + err.message, null); 
      }
    };
  ...
  window.BibliaApp = Object.freeze({ exportNotes, importNotesFromFile, version: '1.4.0' });
  console.info('[Biblia App] v1.4.0 lista. Usa BibliaApp.exportNotes() para respaldar.');
